"""
Wrapper.py - Zhang's Camera Calibration Method
CMSC733 HW1: AutoCalib

Implements:
  1. Checkerboard corner detection
  2. Homography estimation per image (DLT)
  3. Zhang's closed-form intrinsic matrix (K) estimation
  4. Extrinsic (R, t) estimation per image
  5. Radial distortion initialization (k1=k2=0)
  6. Non-linear optimization of all parameters (scipy least_squares)
  7. Image rectification (undistortion)
  8. Reprojection error computation and visualization

Usage:
  python3 Wrapper.py [--img_dir Calibration_Imgs] [--out_dir results]
"""
...
# Configuration
# ============================================================
CHECKER_ROWS = 6        # inner corners along Y (odd-square axis: 7 squares)
CHECKER_COLS = 9        # inner corners along X (even-square axis: 10 squares)
SQUARE_SIZE  = 21.5     # mm
...
def detect_corners(img_paths, pattern_size=(CHECKER_COLS, CHECKER_ROWS)):
    """Detect checkerboard corners in all images using cv2.findChessboardCorners
    with sub-pixel refinement via cv2.cornerSubPix.

    Returns:
        all_corners: list of (N, 2) arrays of detected image points
        img_shapes:  list of (h, w) tuples
        valid_paths: list of image paths where detection succeeded
    """
    ...
    for path in sorted(img_paths):
        ...
        img_shapes.append(gray.shape[:2])
        valid_paths.append(path)
        print(f"[INFO] Detected {len(corners_refined)} corners in {os.path.basename(path)}")

    return all_corners, img_shapes, valid_paths
